...
// Normalize phone numbers to +972XXXXXXXXX for reliable comparisons
const normalizePhone = (phone: string): string => {
  if (!phone) return ''
  let digits = phone.trim()
  // Already +972 format
  if (digits.startsWith('+972')) return digits
  // Handle 972 without plus
  if (digits.startsWith('972')) return `+${digits}`
  // Remove non-digits and convert 0XXXXXXXXX to +972XXXXXXXXX
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-sm p-3 sm:p-4 mb-4 sm:mb-6">
          <div className="space-y-2 sm:space-y-0">
            {/* First Row: Customer Info & Logout Button (and dropdown on desktop) */}
              ...
              <div className="flex-1 min-w-0">
                {phoneNumber && (
                  <div className="flex items-center gap-2">
                    ...
                    {currentSubmission?.form_type_label && (
                      <span className="hidden sm:inline text-xs text-gray-500 border-r pr-2 mr-2 whitespace-nowrap">
                        {currentSubmission.form_type_label}
                      </span>
                    )}
                  </div>
                ...
                {!phoneNumber && (
                  <div className="text-sm text-gray-600">
                    הזן מספר טלפון בכתובת URL
                  </div>
                )}
              </div>
              
              {/* Desktop Form Type Dropdown & Logout Button */}
              <div className="flex items-center gap-2">
                {/* Form Type Dropdown - Desktop Only */}
                <select
                  value={selectedType}
                  onChange={(e) => handleFormTypeChange(e.target.value)}
                  disabled={!!formTypeFromUrl}
                  className={`hidden sm:block text-sm px-3 py-1 border rounded-lg font-medium ${formTypeFromUrl 
                    ? 'border-gray-200 bg-gray-100 text-gray-600 cursor-not-allowed' 
                    : 'border-gray-300 bg-white text-gray-900 hover:bg-gray-50 focus:ring-2 focus:ring-blue-500 focus:border-transparent'
                  }`}
                ...
                {phoneNumber && (
                  <button
                    ...
                    className="text-xs sm:text-sm text-red-600 hover:text-red-800 font-medium bg-red-50 hover:bg-red-100 px-2 sm:px-3 py-1 rounded-lg transition-colors whitespace-nowrap flex-shrink-0"
                  >
                    התנתק
                  </button>
                )}
            ...
            {/* Second Row: Form Type Dropdown (Mobile Only) */}
            <div className="block sm:hidden">
              <select
                value={selectedType}
                onChange={(e) => handleFormTypeChange(e.target.value)}
                disabled={!!formTypeFromUrl}
                className={`w-full text-sm px-3 py-2 border rounded-lg font-medium ${formTypeFromUrl 
                  ? 'border-gray-200 bg-gray-100 text-gray-600 cursor-not-allowed' 
                  : 'border-gray-300 bg-white text-gray-900 hover:bg-gray-50 focus:ring-2 focus:ring-blue-500 focus:border-transparent'
                }`}
                aria-label="בחר סוג טופס"
